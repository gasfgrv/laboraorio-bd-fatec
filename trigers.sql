...
CREATE
OR
replace TRIGGER tg_sufixo BEFORE
INSERT
ON depto FOR EACH ROW BEGIN :new.nomedepto := :new.nomedepto
              || 'sons';
...
CREATE
OR
replace TRIGGER tg_doutor BEFORE
INSERT
OR
UPDATE OF codtit
ON professor FOR EACH ROW BEGIN IF :new.codtit = 1 THEN raise_application_error(-20000, 'Titulação Inválida');
...
CREATE
OR
replace TRIGGER tg_historico AFTER
UPDATE
...
CREATE
OR
replace TRIGGER tg_fkprereq BEFORE
DELETE
ON disciplina FOR EACH ROW DECLARE linhas NUMBER;
...
CREATE
OR
replace TRIGGER tg_fkdisciplina BEFORE
INSERT
OR
UPDATE
