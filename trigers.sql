...
BEGIN
		IF :new.codtit = 1 THEN raise_application_error(-20000, 'Titulação Inválida');
END IF;
END;
...
UPDATE
	ON
	professor FOR EACH ROW DECLARE data_atualizacao TIMESTAMP;
...
INSERT
	INTO
	professor_hist
VALUES ( :new.codprof, :new.coddepto, :new.codtit, :new.nomeprof, data_atualizacao, login_atual );
END;
-- trigger para garantir integridade referencial
 CREATE OR REPLACE TRIGGER tg_fkprereq BEFORE
DELETE
	ON
	disciplina FOR EACH ROW DECLARE linhas NUMBER;
BEGIN
	SELECT
	COUNT(numdisc)
INTO
...
IF linhas > 0 THEN raise_application_error(-20000, 'DELETE na tabela [disciplina] viola a FK pois ha registros em prereq');
END IF;
END;
...
UPDATE
	ON
	prereq FOR EACH ROW DECLARE linhas NUMBER;
BEGIN
	SELECT
	count(numdisc)
INTO
...
IF linhas < 2 THEN raise_application_error(-20000, 'INSERT ou UPDATE na tabela (prereq) viola a FK');
END IF;
END;