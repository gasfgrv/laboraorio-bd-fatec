...
-- e junção natural.SELECT DISTINCT nomedepto
FROM            depto NATURAL
join            horario NATURAL
join            predio
WHERE           horario.anosem = 20021
...
-- correta (porquê?):SELECT DISTINCT nomedepto
FROM            depto NATURAL
...
-- semestre).SELECT DISTINCT nomeprof
...
-- possível usar junção natural).SELECT     disciplina.nomedisc,
           discreq.nomedisc
FROM       disciplina NATURAL
join       prereq
inner join disciplina discreq
ON         prereq.coddeptoprereq = discreq.coddepto
...
-- junções explícitas - quando possível usar junção natural).SELECT    disciplina.nomedisc,
          discreq.nomedisc
FROM      disciplina
left join prereq
...
-- seu pré-requisito.SELECT disciplina.nomedisc,
       dispre.nomedisc
FROM   ( ( disciplina NATURAL
join   prereq )
join   prereq preprereq
ON     (
              prereq.coddeptoprereq = preprereq.coddepto
       AND    prereq.numdiscprereq = preprereq.numdisc ) )
join   disciplina dispre
ON     (
...
-- ALL)SELECT disciplina.nomedisc,
       discpre.nomedisc,
       1 AS nivel
FROM   ( disciplina NATURAL
join   prereq )
join   disciplina discpre
ON     (
...
-- ministraram aulas em 2001/2 (resolver com junção natural).SELECT professor.codprof
FROM   professor NATURAL
join   profturma
WHERE  NOT anosem = 20012
AND    codtit = NULL;SELECT anosem,
       coddepto,
       numdisc,
       siglatur,
       codprof
