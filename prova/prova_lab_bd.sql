...
CREATE
OR replace PROCEDURE p2 IS CURSOR c1 IS
SELECT
  disciplina.nomedisc,
  discpre.nomedisc,
  professor.nomeprof,
...
BEGIN dbms_output.Put_line('iniciando procedure');

OPEN c1;

FETCH c1 INTO disciplinas,
disciplinasprereq,
nomeprof,
anosem;

WHILE c1 % FOUND LOOP dbms_output.Put_line(
  'Nome Disciplina: ' || disciplinas || ' - Nome Pré req:' || disciplinasprereq || ' - Nome Prof: ' || nomeprof || ' ano-semestre: ' || anosem
);
