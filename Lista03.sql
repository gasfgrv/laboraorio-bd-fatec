...
-- (quando possível usar IN, caso contrário usar EXISTS).SELECT DISTINCT nomeprof
FROM            depto,
                professor,
                disciplina,
                profturma
...
-- (subselects),SELECT nomedisc
FROM   depto d NATURAL
join   disciplina NATURAL
join   turma
WHERE  d.nomedepto = 'Informática'
AND    anosem = 20021;

-- b. no estilo cálculo relacional, isto é, com consultas aninhadas (subselects).SELECT nomedisc
FROM   depto d NATURAL
join   disciplina
WHERE  d.nomedepto = 'Informática'
AND    coddepto IN
...
-- colunas deve aparecer vazias.SELECT    coddepto,
          nomedepto,
          numdisc,
          creditodisc,
          siglatur,
...
-- requisito têm a segunda coluna vazia.SELECT    disciplina.nomedisc,
          discreq.nomedisc
FROM      disciplina
left join prereq
ON        disciplina.coddepto = prereq.coddepto
AND       disciplina.numdisc = prereq.numdisc
left join disciplina discreq
ON        discreq.coddepto = prereq.coddeptoprereq
...
-- 6. Obter o número de disciplinas do departamento denominado `Informática'.SELECT Count(nomedisc)
FROM   disciplina NATURAL
join   depto
WHERE  nomedepto = 'Informática';

-- 7. Obter o número de salas que foram usadas no ano-semestre 20021 por turmas do
-- departamento denominado `Informática'.SELECT Count(siglatur)
FROM   horario NATURAL
join   depto
WHERE  nomedepto = 'Informática'
AND    anosem = 20021;
...
-- têm o maior número de créditos dentre as disciplinas deste departamento.SELECT nomedisc
FROM   disciplina NATURAL
join   depto
WHERE  nomedepto = 'Informática'
AND    creditodisc =
       (
              SELECT Max(creditodisc)
              FROM   disciplina );
...
-- disciplinas.SELECT DISTINCT nomedepto,
                (
                       SELECT Count(nomedisc)
                       FROM   disciplina
                       WHERE  disciplina.coddepto = depto.coddepto ) qtdedisc
FROM            depto
ORDER BY        qtdedisc DESC;
...
-- semestre.SELECT DISTINCT nomedepto,
                (
                (
                                SELECT DISTINCT creditodisc
                                FROM            disciplina
                                WHERE           disciplina.coddepto = depto.coddepto ) *
                (
                       SELECT Count(siglatur)
                       FROM   horario
                       WHERE  horario.coddepto = depto.coddepto ) ) AS qtdedisc
FROM            depto
GROUP BY        nomedepto
ORDER BY        qtdedisc DESC;

-- 11. Resolver a consulta da questão anterior, mas incluindo somente os
-- departamentos que têm mais que 5 disciplinas.SELECT DISTINCT nomedepto,
                (
                (
                                SELECT DISTINCT creditodisc
                                FROM            disciplina
...
WHERE           (
                       SELECT Count(nomedisc)
                       FROM   disciplina
                       WHERE  disciplina.coddepto = depto.coddepto ) > 5
GROUP BY        nomedepto
ORDER BY        qtdedisc DESC;

-- 12. Obter os nomes dos departamentos que possuem a maior soma de créditos.SELECT nomedepto    nomedepto,
       somacreditos somacreditos
FROM   (
              SELECT nomedepto,
                     (
                            SELECT SUM(creditodisc)
                            FROM   disciplina
                            WHERE  disciplina.coddepto = depto.coddepto ) somacreditos
              FROM   depto ) tbl
WHERE  tbl.somacreditos >=
       (
                SELECT   Max(SUM(creditodisc))
                FROM     disciplina NATURAL
                join     depto
                GROUP BY coddepto );
...
-- Resolver a questão da seguinte forma:
-- 1. sem usar GROUP BY, com consultas aninhadas (subselects),SELECT nomedisc
FROM   disciplina
WHERE  nomedisc =
       (
              SELECT nomedisc
...
-- 2. usando GROUP BY, sem consultas aninhadas.SELECT nomedisc
FROM   horario NATURAL
join   disciplina
WHERE  disciplina.creditodisc <> horario.numhoras;

-- 14. Obter os nomes dos professores que, em 20022, deram aula em mais de uma
-- turma.
-- Resolver a questão da seguinte forma:
-- 1. sem funções de agregação (tipo COUNT, MIN,MAX,AVG,SUM),SELECT DISTINCT nomeprof
FROM            profturma,
                professor
WHERE           anosem = 20022
ORDER BY        nomeprof ASC;

-- 2. ou ainda, com funções de agregação.SELECT DISTINCT nomeprof,
                Count(siglatur)
FROM            profturma,
                professor
WHERE           anosem = 20022
